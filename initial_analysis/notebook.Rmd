...
```{r, fig.width = 12, fig.height = 18, fig.retina = FALSE, eval = FALSE}
top_projects <- names(head(sort(table(data$project), decreasing = TRUE), 12))
png("~/Desktop/abc_proj.png", width = 9, height = 12, unit = "in", res = 100)
par(mfrow = c(4, 3))
for ( top_project in top_projects ) {
  dplyr::filter_(data, lazyeval::interp(~project == proj, proj = top_project)) %>%
    with(table(group, outcome)) %>% t %>% mosaicplot(main = top_project, col = scales::hue_pal()(3), las = 1, cex.axis = 1)
}
dev.off()
...
```{r, fig.width = 12, fig.height = 18, fig.retina = FALSE, eval = FALSE}
top_languages <- names(head(sort(table(data$language), decreasing = TRUE), 12))
...
for ( top_language in top_languages ) {
  dplyr::filter_(data, lazyeval::interp(~language == lang, lang = top_language)) %>%
    with(table(group, outcome)) %>% t %>% mosaicplot(main = top_language, col = scales::hue_pal()(3), las = 1, cex.axis = 1)
}
dev.off()
